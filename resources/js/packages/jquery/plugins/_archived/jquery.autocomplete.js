...
(function($){
	...
	$.fn.autocomplete = function(obj){
		if($.browser.msie) { obj.mode = 'outer'; }
	...
	/*function breakLongLines(lines,mirror){
		var ret = [];
		for( var i=0; i<lines.length; i++ ){
			var lines2 = breakLine(lines[i],mirror);
	...
	function breakLine(line,mirror){
		 var _start = new Date().getTime();
		 stats.breakLine.runCount++;
		 var ret =[line];
		 var count=0;
		 mirror.html("<span style='display:inline-block;'>"+line+"</span>");
		 var span = mirror.children("span");
		 if( span.width()+10 < mirror.width()) return [line];
		 while( span.width() >= mirror.width()-10 ){
		 	
		 	count++;
		 	var words = line.split(" ");
		 	var left = words.slice(0,words.length-count);
		 	var right = words.slice(words.length-count,words.length);
		 	mirror.html("<span style='display:inline-block;'>"+left.join(" ")+"</span>");
		 	var span = mirror.children("span");
		 	
		 	ret = [left.join(" "),right.join(" ")];		 	
		 }
		 var arr = breakLine(ret[1],mirror);
		 var ret2= [ret[0]];
	...
	function getCursorPosition(data){
		...
		for( var i=0; i< lines.length-1; i++){
			mirror.append("<div style='height:"+(parseInt(data.lineHeight))+"px"+";'>"+lines[i]+"</div>");
		}
		mirror.append("<span id='"+data.id+"' style='display:inline-block;'>"+lines[lines.length-1]+"</span>");
		
		mirror.append("<span id='rest' style='max-width:'"+data.ta.clientWidth+"px'>"+restText.replace(/\n/g,"<br/>")+"&nbsp;</span>");
		
		mirror.get(0).scrollTop = ta.scrollTop;
		
		var span = mirror.children("#"+data.id);
