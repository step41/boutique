...
(function (factory) {
  ...
  $.fn.perfectScrollbar = function (suppliedSettings, option) {
    ...
    return this.each(function () {
	  ...
	  // Step41 added the following to ensure overflow was hidden prior to implementation.
	  // This prevents the native scrollbar from leaving a visible gap on the container when it disappears.
	  // There is a difference between the perfect scrollbar width and native which makes this quite ugly.
      ...
      var bindKeyboardHandler = function () {
        ...
        $this.bind('mouseenter.perfect-scrollbar', function (e) {
          hovered = true;
		  /** Jeff Todnem added the following custom code to support moving certain elements over when scrollbar is visible **/
			if (scrollbarYActive && settings.vertShift) { 
		  		$this.thisPaddingLeft = ($this.thisPaddingLeft) ? $this.thisPaddingLeft : $this.css('padding-left'); 
				$this.animate({paddingLeft:$scrollbarY.css('width')}); 
			}
		  /** End Jeff Todnem's code **/
        });
        $this.bind('mouseleave.perfect-scrollbar', function (e) {
          hovered = false;
		  /** Jeff Todnem added the following custom code to support moving certain elements over when scrollbar is visible **/
		  if ($this.thisPaddingLeft && settings.vertShift) { $this.animate({paddingLeft:$this.thisPaddingLeft}); }
		  /** End Jeff Todnem's code **/
        });
