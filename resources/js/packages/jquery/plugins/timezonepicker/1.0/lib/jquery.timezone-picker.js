	...
	return $origCall.each(function(index, item) {
		...
		$(mapElement).find('area').click(function() {
			...
			if ($pin) {
				...
				var dims = $pin.hiddendims();
				
				var pinWidth = parseInt(dims.outerWidth / 2);
				var pinHeight = dims.outerHeight;
		...
		/* Jeff Todnem - Step41 - 20150306 - Added configured to avoid unnecessary re-binding */
		 $origCall.configured = true;
	});
};
